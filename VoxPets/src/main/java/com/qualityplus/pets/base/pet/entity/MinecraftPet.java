...
public abstract class MinecraftPet implements PetEntity {
    ...
    public void spawn() {
        ...
        this.pet.getCommandsWhenSpawn(getLevel())
                .forEach(commandReward -> commandReward.execute(getPlayer()));
    }
    ...
    private void handleSpawn() {
        ...
        Optional<UserData> data = VoxPets.getApi().getUsersService().getData(offPlayer.getUniqueId());
    ...
    protected void handleDeSpawnReason(DeSpawnReason deSpawnReason) {
        ...
        Optional<UserData> userData = VoxPets.getApi().getUsersService().getData(offPlayer.getUniqueId());
    ...
    protected void removeStats() {
        ...
        this.pet.getCommandWhenDeSpawn(level)
                .forEach(commandReward -> commandReward.execute(player));
    }

    private int getLevel() {
        Optional<PetData> petData = VoxPets.getApi().getPetsService().getData(petUniqueId);
