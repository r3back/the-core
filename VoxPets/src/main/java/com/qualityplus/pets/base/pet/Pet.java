...
public final class Pet extends OkaeriConfig {
    ...
    public List<CommandReward> getCommandsWhenSpawn(int level) {
        return getPetLevel(level)
                .map(PetLevel::getCommandsWhenSpawn)
                .orElse(Collections.emptyList());
    }

    public List<CommandReward> getCommandWhenDeSpawn(int level) {
        return getPetLevel(level)
                .map(PetLevel::getCommandWhenDeSpawn)
                .orElse(Collections.emptyList());
    }
