...
public final class DatabaseFactory extends HikariConfiguration {
    ...
    public DocumentPersistence configurePersistence(@Inject("dataFolder") File dataFolder, @Inject Config config) {
        ...
        switch (backend) {
            ...
            case H2:
                return new DocumentPersistence(new H2Persistence(basePath, getH2Hikari("VoxBank")), JsonSimpleConfigurer::new, new SerdesBukkit());
            case REDIS:
            ...
            /*case MONGODB:
                MongoClientURI clientURI = new MongoClientURI(parseDatabaseUrl(db));

                MongoClient mongoClient = new MongoClientImpl(clientURI);
