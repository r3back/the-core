...
public final class CollectionGUI extends CollectionsGUI {
    ...
    public void onInventoryClick(InventoryClickEvent event) {
        ...
        if (isItem(slot, config.getCloseGUI())) {
            ...
            final String command = this.config.getGoBack().getCommand();
            if (command != null && !command.isEmpty()) {
                handleItemCommandClick(player, this.config.getGoBack());
            } else {
                box.files().categories()
                        .getById(collection.getCategory())
                        .ifPresent(category -> player.openInventory(new CategoryGUI(box, player, category).getInventory()));
            }
