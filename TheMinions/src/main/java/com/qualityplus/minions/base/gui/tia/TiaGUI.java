package com.qualityplus.minions.base.gui.tia;
...
public final class TiaGUI extends SoulsGUI {
    ...
    private List<IPlaceholder> getPlaceholders(SoulsData soulsData){
        ...
        String placeholder = collected >= max ? box.files().messages().minionMessages.clickToClaim : box.files().messages().minionMessages.notEnoughSoulsPlaceholder;
    ...
    public void onInventoryClick(InventoryClickEvent event) {
        ...
        if(isItem(slot, config.getCloseGUI())){
            ...
            if(collected < max){
                player.sendMessage(StringUtils.color(box.files().messages().minionMessages.notEnoughSoulsMessage));
                return;
            }
