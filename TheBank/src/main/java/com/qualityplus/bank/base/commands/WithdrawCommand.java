...
@Component
public final class WithdrawCommand extends AssistantCommand {
    ...
    public boolean execute(CommandSender sender, String[] args) {
        if (args.length == 3) {
            ...
            final BankTransaction transaction = new BankTransaction(amount, TransactionType.WITHDRAW, BankInterfaceGUI.GUIType.GENERAL, TransactionCaller.SERVER);
            ...
            sender.sendMessage(StringUtils.processMulti("&aYou successfully withdraw %amount% to %player%'s bank", placeholders));
        } else
    ...
    public void register(@Inject Box box) {
        TheAssistantPlugin.getAPI().getCommandProvider().registerCommand(this, e -> e.getCommand().setDetails(box.files().commands().withdrawBankCommand));
    }
}