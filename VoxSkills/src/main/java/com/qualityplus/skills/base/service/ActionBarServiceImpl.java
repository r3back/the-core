...
public final class ActionBarServiceImpl implements ActionBarService {
    private final Cache<UUID, Boolean> actionBarCache = CacheBuilder.newBuilder()
            .concurrencyLevel(4)
            .maximumSize(3000)
    ...
    public void sendActionBar(Player player, String message) {
        actionBarCache.put(player.getUniqueId(), true);
    ...
    public void startScheduling(@Inject SkillsService service, @Inject Config config) {
        ...
        for (Player player :  Bukkit.getOnlinePlayers()) {
            ...
            if (this.actionBarCache.getIfPresent(player.getUniqueId()) != null) {
                continue;
            }
