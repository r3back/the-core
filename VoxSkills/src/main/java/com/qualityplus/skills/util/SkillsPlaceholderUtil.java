...
public class SkillsPlaceholderUtil {
    ...
    private static List<IPlaceholder> getCommonPlaceholders(CommonObject object, double level) {
        ...
        String romanNumber = String.valueOf(level);
        try {
            romanNumber = NumberUtil.toRoman((int)level);
        } catch (Exception e) {
        ...
        placeholders.add(new Placeholder("skill_level_" + object.getId() + "_roman", romanNumber));
    ...
    public static PlaceholderBuilder getHealthPlaceholders(Player player) {
        final boolean scaleHealth = VoxSkills.getApi().getConfig().healthSettings.scaledHealthEnabled;
        final int scale = VoxSkills.getApi().getConfig().healthSettings.multiplyHealthFor;
        final String maxHealth = NumberUtil.toInt(player.getMaxHealth() * (scaleHealth ? scale : 1));
        final String health = NumberUtil.toInt(player.getHealth() * (scaleHealth ? scale : 1));

        return PlaceholderBuilder.create()
                .with(new Placeholder("player_max_health", maxHealth))
                .with(new Placeholder("player_health", health));
    }
