...
public class MinionLayoutServiceImpl implements MinionLayoutService {
    ...
    public LayoutType getMinionLayout(final MinionEntity entity) {
        ...
        final Optional<MinionData> data = VoxMinions.getApi().getMinionsService().getData(entity.getMinionUniqueId());
        if (data.isEmpty()) {
            return layoutType;
        }
    ...
    private boolean hasMinionExpander(final UpgradeEntity entity) {
        ...
        final MinionUpgrade upgrade = VoxMinions.getApi().getConfigFiles().upgrades().normalUpgrades.getOrDefault(entity.getId(), null);
        if (upgrade == null) {
            return false;
        }
