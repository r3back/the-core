...
import com.qualityplus.minions.base.recipe.provider.VoxCraftingRecipeProvider;
...
public final class RecipeProviderFactory {
    ...
    public RecipeProvider createRecipeProvider() {
        if (TheAssistantPlugin.getAPI().getDependencyResolver().isPlugin("VoxCrafting")) {
            return injector.createInstance(VoxCraftingRecipeProvider.class);
        } else
