...
public final class FuelItemSetup implements ItemSetup {
    ...
    public void setItem(Inventory inventory, Box box, MainMinionGUIConfig config, MinionEntity minionEntity) {
        ...
        final FuelEntity fuelEntity = minionData
        ...
        if (fuelEntity == null)
            ...
            ItemStack itemStack = fuel.getItemStack(
                    fuelEntity.getMarkable().getDelay(),
                    fuelEntity.getMarkable().getLastMarked(),
                    fuelEntity.getRemainingSeconds()
            );
    ...
    private String getRemainingTime(FuelEntity fuelEntity) {
        Messages messages = VoxMinions.getApi().getConfigFiles().messages();
