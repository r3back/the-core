...
public abstract class ArmorStandMinion<T> extends MinecraftMinion implements Listener {
    ...
    public void unloadMinion(
        ...
        if (saveDBAsync) {
            Bukkit.getScheduler().runTaskAsynchronously(VoxMinions.getInstance(), () -> data.get().save());
        } else {
            data.get().save();
        }
    }
    ...
    public Collection<ItemStack> pickUpAllItems() {
        Optional<MinionData> minionData = VoxMinions.getApi().getMinionsService().getData(state.getUuid());
    ...
    private void executeStartAnimation() {
        ...
        this.startAnimation.executeAnimation(context).thenRun(() -> Bukkit.getScheduler().runTaskLater(VoxMinions.getInstance(), () -> this.state.setCanExecuteAnimation(true), 20));
    }
    ...
    private void updateStatus() {
        ...
        boolean hasInvalidLayout = state.isArmorStandLoaded() ? VoxMinions.getApi().getMinionLayoutService().hasInvalidLayout(this) : false;
        ...
        VoxMinions.getApi().getMinionDisplayNameService().updateDisplayName(this);
    }
