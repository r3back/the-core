...
public final class SellHandlerImpl implements SellHandler, LevelGetter {
    ...
    public void sellIfItsPossible() {
        ...
        Optional<MinionData> minionData = VoxMinions.getApi().getMinionsService().getData(petUniqueId);
        ...
        double percentage = Optional.ofNullable(VoxMinions.getApi().getConfigFiles().getAutoSell().automatedShippingUpgrades.getOrDefault(automatedShipping.getId(), null))
        ...
        Bukkit.getScheduler().runTaskLater(VoxMinions.getInstance(), () -> state.setSelling(false), eachTimer);
    }
