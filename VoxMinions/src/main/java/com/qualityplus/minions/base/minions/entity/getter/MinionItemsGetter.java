...
public interface MinionItemsGetter {
    ...
    default String getUpgrade(UpgradeSlot slot, UUID minionUniqueId) {
        Optional<MinionData> data = VoxMinions.getApi().getMinionsService().getData(minionUniqueId);
        ...
        MinionUpgrade upgrade = VoxMinions.getApi().getConfigFiles().upgrades().normalUpgrades.getOrDefault(id, null);
