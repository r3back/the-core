...
public final class PickUpMinionClickHandler implements ClickHandler {
    ...
    private void deSpawn(Player player, MinionEntity entity, Box box) {
        ...
        Optional<UserData> data = VoxMinions.getApi().getUserService().getData(player.getUniqueId());
    ...
    private void dropMinionItems(Player player, MinionEntity minionEntity, Box box) {
        ...
        final Optional<MinionData> minionData = VoxMinions.getApi().getMinionsService().getData(minionEntity.getMinionUniqueId());
        ...
        if (fuel != null) {
            ...
            if (minionUpgrade != null) {
                player.getWorld().dropItem(
                        player.getLocation(), minionUpgrade.getItemStack(
                                fuel.getMarkable().getDelay(),
                                fuel.getMarkable().getLastMarked(),
                                fuel.getRemainingSeconds()
                        )
                );
            }
