...
public final class EnchantSubGUI extends EnchantingGUI {
    ...
    public @NotNull Inventory getInventory() {
        ...
        for (final Integer slot : slots) {
            ...
            if (enchantment.canEnchant(player, level) || box.files().config().showEnchantmentIfPlayerDoesntHavePermission) {
    ...
    public void onInventoryClick(final InventoryClickEvent event) {
        ...
        if (isItem(slot, config.getCloseGUI())) {
            ...
            final VoxEnchantingEnchantEvent event1 = new VoxEnchantingEnchantEvent(player, session);
            ...
            final ItemStack itemStack = isRemoveEnchant ? VoxEnchanting.getApi().removeEnchantment(item, session) : VoxEnchanting.getApi().addEnchantment(item, session);
    ...
    private List<IPlaceholder> getWarnings(int level) {
        ...
        final Optional<ICoreEnchantment> incompatibility = VoxEnchanting.getApi().getIncompatibility(item, enchantment);
