...
public final class DragonControllerImpl implements DragonController {
    ...
    public void target() {
        ...
        if (MathUtil.offset(this.location, this.target) < 4.0D) {
            Optional<DragonSpawn> spawn = VoxDragon.getApi().getStructureService().getSpawn();
    ...
    public void targetSky() {
        ...
        VoxDragon.getApi().getPlugin().getServer().getPluginManager().callEvent(new DragonTargetEvent(null, this, TargetType.SKY));
    }
    ...
    public void targetPlayer() {
        this.rangeBest = 9000.0D;
        this.rangeTime = System.currentTimeMillis();
        final int randomIndex = Math.max(RandomUtil.randomUpTo(VoxDragon.getApi().getUserService().getUsers().size() - 1), 0);
        final Player player = VoxDragon.getApi().getUserService().getUsers().get(randomIndex).getPlayer();
        this.target = player.getLocation();
        this.target.add(DragonVelocityUtil.getTrajectory(this.location, this.target).multiply(3.5D));
        VoxDragon.getApi().getPlugin().getServer().getPluginManager().callEvent(new DragonTargetEvent(player, this, TargetType.PLAYER));
    }
