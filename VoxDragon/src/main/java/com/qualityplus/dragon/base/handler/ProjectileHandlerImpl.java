    ...
    private void shoot(Player player, ProjectileType projectile, double damage, int i, DragonGame dragonGame) {
        Bukkit.getScheduler().runTaskLater(VoxDragon.getApi().getPlugin(), () -> {
            if (dragonGame.isActive() && player != null && player.isOnline()) {
                final EnderDragon enderDragon = VoxDragon.getApi().getDragonService().getActiveEnderDragon();
                ...
                projectile1.setMetadata("DragonFireball", new FixedMetadataValue(VoxDragon.getApi().getPlugin(), damage));
