...
public final class DragonGuardianEvent extends DragonGameEvent {
    ...
    public void start(final DragonGame dragonGame) {
        time = 0;
        task = Bukkit.getScheduler().runTaskTimerAsynchronously(VoxDragon.getApi().getPlugin(), () -> {
            move(dragonGame);
            //Cancelling Event
            if (time >= duration) {
                finish();
    ...
    public void finish() {
        ...
        guardians.stream()
                ...
                .forEach(entity -> Bukkit.getScheduler().runTask(VoxDragon.getApi().getPlugin(), entity::remove));
    }
    ...
    private void spawn() {
        ...
        locations.forEach(loc -> Bukkit.getScheduler().runTask(VoxDragon.getApi().getPlugin(), () -> addGuardian(loc)));
    }
    ...
    private Set<Location> getRandomLocations(final int amount) {
        ...
        for (int i = 0; locations.size()<amount; i++) {
            Location location = VoxDragon.getApi().getGuardianService().getRandomLocation();
    ...
    private void addGuardian(final Location location) {
        try {
            final Guardian guardian = VoxDragon.getApi().getGuardianService().getRandomGuardian(this.guardiansConfig);
