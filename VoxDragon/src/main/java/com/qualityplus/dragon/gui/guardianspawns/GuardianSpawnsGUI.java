...
public final class GuardianSpawnsGUI extends TheDragonGUI {
    private final Map<Integer, ALocation> crystalMap = new HashMap<>();
    ...
    public @NotNull Inventory getInventory() {
        ...
        List<ALocation> crystals = box.files().guardians().guardianSpawns;
        int slot = 0;
        ...
        if (!crystals.isEmpty()) {
            while (slot < maxPerPage) {
                if (crystals.size() > i && i >= 0) {
                    ALocation location = crystals.get(i);

                    List<IPlaceholder> placeholders = PlaceholderBuilder.create(
                            new Placeholder("thedragon_guardian_spawn_location", LocationUtils.toString(location.getLocation())),
    ...
    public void onInventoryClick(InventoryClickEvent e) {
        ...
        if (isItem(slot, config.getCloseGUI())) {
            ...
            ALocation toRemove = crystalMap.getOrDefault(slot, null);
