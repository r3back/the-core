...
public final class ParticleHandlerImpl implements ParticleHandler {
    @Override
    public void spell(DragonGame dragonGame, ParticleEffect particle) {
        int points = 30;
        ...
        Bukkit.getScheduler().runTaskLater(TheDragon.getApi().getPlugin(), () -> {
            ...
            final Location origin = TheAssistantPlugin
                    .getAPI()
                    .getNms()
                    .getDragonPart(TheDragon.getApi().getDragonService().getDragonController().dragon(), NMS.DragonPart.BODY)
                    .clone()
                    .subtract(0,3,0);
